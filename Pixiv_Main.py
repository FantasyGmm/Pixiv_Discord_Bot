...
def search(img_id):
    ...
    if imgurl[1] == 'illust':
        ...
        imginfo = Pixiv_Lib.PixivV1_img_name(type=imgurl[1], id=img_id)
        return imginfo
    if imgurl[1] == 'ugoira':
        ...
        if imgurl[len(imgurl) - 1] == 'illustration':
            while i < (len(imgurl) - 1):
                print('download', i)
                Pixiv_Lib.download_imgs(img_id, imgurl[i], i)
                i += 1
            imginfo = Pixiv_Lib.PixivV1_img_name(type=imgurl[int(len(imgurl) - 1)], id=img_id)
            return imginfo


def clean():
    Pixiv_Lib.clea_imgcache()
